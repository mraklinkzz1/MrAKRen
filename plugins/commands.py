...
async def start(client, message):
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('🚀 Fast Download / Watch Online1🖥️1', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @I_Am_MrAK
                            ]
                        ]
                    )
                )
    ...
    if data.startswith("sendfiles"):
        ...
        if not await db.has_premium_access(user_id) and settings['is_shortlink']: #Don't change anything without my permission @I_Am_MrAK
            files_ = await get_file_details(file_id)
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : 𝙼𝚁𝗔𝗞 𝗟𝗶𝗻𝗸𝗭𝘇')
    ...
    if settings is not None:
        buttons = [        
            ...
            [
                ...
                InlineKeyboardButton(
                    '8' if settings["max_btn"] else f'{MAX_B_TN}',
                    callback_data=f'setgs#max_btn#{settings["max_btn"]}#{grp_id}',
                ),
            ],
            [
...
async def settutorial(bot, message):
    ...
    if len(message.command) == 1:
        ...
        return await message.reply("<b>ʏᴏᴜ ᴇɴᴛᴇʀᴇᴅ ɪɴᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ !\nᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ : /set_tutorial <code>https://t.me/MrAK_LinkZz/5</code></b>")
